...
## ⚠️ 문서(.md) 자율 수정 규칙 (MANDATORY)

**문서(.md) 파일 수정은 사용자 허락 요청 없이 즉시 수행한다.**

- ✅ 명세 업데이트, 결정 기록, 새 섹션 작성, 표 갱신 — 모두 자율
- ✅ 디자인-명세 불일치 발견 시 즉시 명세 수정
- ✅ 합리적 결정 사항 즉시 적용 (단순화·명백한 패턴)
- ⚠️ 작업 완료 후 변경 사항 명확히 보고
- ⚠️ 다음만 예외 (명시적 사용자 동의 필요):
  - 코드 작성·실행
  - 커밋·푸시·배포 (사용자가 명시 요청 시에만)
  - 데이터 삭제·되돌리기 어려운 작업
  - 비즈니스 결정 (가격·전략 등)
  - 진짜 옵션 A vs B가 동등하여 사용자 의견 필요한 경우만
